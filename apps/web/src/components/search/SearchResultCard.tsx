...
import type { SearchResultItem, TagColor, TagWithCount } from 'shared';
...
export function SearchResultCard({ result, tags }: SearchResultCardProps) {
  const { note, headline } = result;
  ...
  return (
    ...
    >
      ...
      <p
        className="mt-1 text-sm text-slate-600 [&_mark]:bg-yellow-200 [&_mark]:text-slate-900"
      ...
      <p className="mt-3 text-xs text-slate-400">Updated {formatRelativeTime(note.updatedAt)}</p>
    </Link>
  );
}
