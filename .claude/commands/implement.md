...
4. Ensure openspec/changes/$ARGUMENTS/review-log.md exists.
5. Ensure openspec/changes/$ARGUMENTS/fix-bundles.md exists.
6. CRITICAL NOTEAPP RULES:
...
EXECUTION LOOP — for each unchecked task in openspec/changes/$ARGUMENTS/tasks.md, in order:
  ...
  STEP 4 — Triage findings
    Combine tester failures + reviewer findings from openspec/changes/$ARGUMENTS/review-log.md.
    - Case A: All [OK] → mark task done.
    - Case B: Tester failures OR [WARN]/[FAIL] findings → Construct FIX BUNDLE inside fix-bundles.md. Present to user with ONE approval.
  ...
  STEP 5 — After all tasks done
    ...
    Verify spec.md, FRS.md, code in sync.
    STOP: "Implementation complete. Run openspec archive $ARGUMENTS then /pr."
